...
# to one of the rooms (i + 1, j - 1), (i + 1, j), and (i + 1, j + 1) if the
# target room exists.
# The child starting from (n - 1, 0) must move from their current room (i, j)
# to one of the rooms (i - 1, j + 1), (i, j + 1), and (i + 1, j + 1) if the
# target room exists.
# 
# 
# When a child enters a room, they will collect all the fruits there. If two or
# more children enter the same room, only one child will collect the fruits,
...
class Solution:
    def maxCollectedFruits(self, fruits: List[List[int]]) -> int:
        ...
        dp2 = [[0] * col for _ in range(row)]
        dp1[0][col-1] = fruits[0][col-1]
        for i in range(1,row):
            for j in range(col-1,col - i - 2,-1):
                for a,b in [(-1,-1),(-1,0),(-1,1)]:
                    newr = i + a
                    newc = j + b
                    if 0 <= newr < len(fruits) and 0 <= newc < len(fruits[0]):
                        dp1[i][j] = max(dp1[i][j],dp1[newr][newc])
                dp1[i][j] += fruits[i][j]
        dp2[row-1][0] = fruits[row-1][0]
        for i in range(1,col):
            for j in range(row - 1, row - i - 2,-1 ):
                for a,b in [(1,-1),(0,-1),(-1,-1)]:
                    newc = i + b
                    newr = j + a
                    if 0 <= newr < len(fruits) and 0 <= newc < len(fruits[0]):
                        dp2[j][i] = max(dp2[j][i],dp2[newr][newc])     
                dp2[j][i] += fruits[j][i]
        return res + dp1[row-1][col-1] + dp2[row-1][col-1]
...
#
# @lcpr case=start
# [[1,1000,3,4],[5,6,8,7],[9,10,11,12],[13,14,15,16]]\n
# @lcpr case=end
